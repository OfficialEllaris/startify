<div class="flex min-h-screen items-center justify-center px-4 py-8 sm:py-12">
    <div class="w-full max-w-md">
        ...
        <a href="{{ route('web.home') }}" class="flex items-center justify-center gap-4 mb-12 group/logo hover:opacity-80 transition-opacity">
            <div
                class="w-12 h-12 bg-primary rounded-2xl flex items-center justify-center text-primary-content shadow-2xl shadow-primary/30 rotate-3 transition-transform group-hover/logo:scale-110 group-hover/logo:rotate-6">
                <img src="{{ asset('favicon.ico') }}" alt="{{ config('app.name') }}" class="w-7 h-7 -rotate-3">
            </div>
            <div>
                <h1 class="text-2xl font-black tracking-tighter uppercase italic leading-none text-base-content">{{ config('app.name') }}
                </h1>
